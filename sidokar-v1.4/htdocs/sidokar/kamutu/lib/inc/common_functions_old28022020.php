...
function menu()
{
    ...
    $sql_1st = "SELECT DISTINCT * FROM
                (
                    SELECT modules_id,modules_title,modules_link,modules_icon
                    FROM modules 
                    INNER JOIN user_group_access ON modules_id = access_module_id
                    WHERE modules_category IN(1) AND `modules_parent` IN(0) AND access_group_id IN (".$_SESSION['user_group_id'].") AND access_record_status='A' AND modules_is_display = 1 AND modules_position = 'left' AND modules_record_status = 'A'
                    
                    UNION
                    
                    SELECT modules_id,modules_title,modules_link,modules_icon
                    FROM modules 
                    INNER JOIN user_access ON modules_id = access_module_id AND access_record_status = 'A'
                    WHERE modules_category IN(1) AND `modules_parent` IN(0) AND access_user_id = '{$_SESSION['user_profile_id']}' AND access_record_status='A' AND modules_is_display = 1 AND modules_position = 'left' AND modules_record_status = 'A'
    ...
    if (mysqli_num_rows($result1) > 0) {
        while($level_1st = mysqli_fetch_assoc($result1)){
           ...
           $sql_2nd = "SELECT DISTINCT * FROM
                    (
                        SELECT modules_id,modules_title,modules_link FROM modules 
                        INNER JOIN user_group_access ON modules_id = access_module_id
                        WHERE modules_parent='$menu_id_1st' AND modules_is_display = 1 AND access_group_id IN (".$_SESSION['user_group_id'].") AND modules_record_status = 'A'
                        UNION
                        SELECT modules_id,modules_title,modules_link FROM modules 
                        INNER JOIN user_access ON modules_id = access_module_id AND access_record_status = 'A'
                        WHERE modules_parent='$menu_id_1st' AND modules_is_display = 1 AND access_user_id IN (".$_SESSION['user_profile_id'].") AND modules_record_status = 'A'
                        ORDER BY modules_id ASC
                    )o";
           ...
           {
                $menu .= "
                    <li> <a class='has-arrow waves-effect waves-dark' href='javascript:void(0)' aria-expanded='false'><i class='$modules_icon_1st'></i><span class='hide-menu'>$modules_title_1st</span></a>
                ...
                while ($level_2nd = mysqli_fetch_assoc($result2)) {
                    ...
                    $sql_3rd = "SELECT DISTINCT * FROM
                                (
                                    SELECT modules_id,modules_title,modules_link FROM modules 
                                    INNER JOIN user_group_access ON modules_id = access_module_id
                                    WHERE modules_parent='$menu_id_2nd' AND modules_is_display = 1 AND access_group_id IN (".$_SESSION['user_group_id'].") AND modules_record_status = 'A'
                                    UNION
                                    SELECT modules_id,modules_title,modules_link FROM modules 
                                    INNER JOIN user_access ON modules_id = access_module_id AND access_record_status = 'A'
                                    WHERE modules_parent='$menu_id_2nd' AND modules_is_display = 1 AND access_user_id IN (".$_SESSION['user_profile_id'].") AND modules_record_status = 'A'
                                    ORDER BY modules_id ASC
                                ) o";
            	    $result3 = mysqli_query($conn,$sql_3rd);
                    if (mysqli_num_rows($result3) > 0) {
                        $menu .= "
                                    <li> <a class='has-arrow' href='javascript:void(0)' aria-expanded='false'>$modules_title_2nd</a>
                                    <ul aria-expanded='false' class='collapse'>";
                        ...
                        while ($level_3rd = mysqli_fetch_assoc($result3)) {
                            ...
                            $menu .= "<li><a href='$modules_link_3rd'>$modules_title_3rd</a></li>";
                    ...
                    {
                        $menu .= "<li><a href='$modules_link_2nd'>$modules_title_2nd</a></li>";
                    }
                }
            ...
            else $menu .= "<li> <a href='$modules_link_1st' aria-expanded='false'><i class='$modules_icon_1st'></i><span class='hide-menu'>$modules_title_1st</span></a></li>";
