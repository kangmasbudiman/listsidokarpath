...
{        
   ...
   $sql="SELECT * 
        FROM applicant_user
        LEFT JOIN applicant_user_profile ON profile_id = user_profile_id
        WHERE user_name = '".$field['user_name']."' AND user_password = '".md5($field['user_password'])."' AND user_record_status = 'A'";
  ...
  {                   
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            $_SESSION['user_institution_code'] = $row['user_institution_code'];
            ...
            $_SESSION['user_province_code'] = $row['user_province_code'];   
            $_SESSION['user_city_code'] = $row['user_city_code']; 
        ...
        //Baca foto
        $sql_photo = "SELECT * FROM `applicant_user_profile_document` 
        ...
        { 
            WHILE ($row_access = mysqli_fetch_assoc($result_photo))
            {
                ...
                if($width > 150)
                {
                    ...
                    $sql_photo = "UPDATE `applicant_user_profile_document` SET document_filename = '{$_SESSION['user_fullname']}.$ext'
                                    WHERE document_type_id IN('4') AND document_requirement_id IN('1') AND `document_profile_id` IN('{$_SESSION['user_profile_id']}') AND `document_record_status` = 'A'";
                    $result_photo = mysqli_query($conn,$sql_photo);
        ...
        $sql_login = "UPDATE applicant_user SET user_last_login = '".date('Y-m-d h:i:s')."', user_online_status='1', user_amount_login=COALESCE(user_amount_login+1,1), user_session='".session_id()."' WHERE user_profile_id = '{$_SESSION['user_profile_id']}'";
